const list_items = [
  ...
  function Displayitems(items,wrapper,rows_per_page,page){
      ...
      for(let i = 0; i < paginatedItems.length; i++){
          let items = `Idno:${paginatedItems[i].id} 
          Name:${paginatedItems[i].name} 
          Email:${paginatedItems[i].email}
          
          `;

          let items_element = document.createElement("div");
          items_element.setAttribute("id","items");
          items_element.classList.add("items");
          items_element.innerText = items;
    ...
    function PaginationButton(page,items){
        ...
        button.addEventListener("click", function(){
            current_page = page;
            Displayitems(list_items,list_element,rows,current_page);
        });
        return button;
